...
Func _OOPE_ParseAndRun()
	...
	$sSourceCode = StringReplace($sSourceCode, @LF, @CRLF)

	$bHasMacros = _OOPE_ContainsMacros($sSourceCode) > 0
	; (1) Run the macro extender
	If $bHasMacros Then _OOPE_ExtendMacros($sSourceCode)

	$bHasClasses = _OOPE_ContainsClasses($sSourceCode) > 0
	...
	If $bHasMacros Or $bHasClasses Then
		_OOPE_WriteAndRun(StringRegExpReplace($sSourceCode, '(\R|\h+\R|\v+\R){2,}+', @CRLF & @CRLF), $sGeneratedScript)
	Else
...
Func _OOPE_WriteAndRun($sCode, $sFile)
	$sCode = ";" & @ScriptName & " ==> Generated by OOPE " & _Now() & @CRLF & $sCode
	Local $bIsConsole = StringInStr($sCode, Chr(0x23) & 'AutoIt3Wrapper_Change2CUI=y') Or StringInStr($sCode, 'ConsoleWrite')
	$sCode = StringReplace($sCode, Chr(0x23) & 'AutoIt3Wrapper_Run_AU3Check=n', ($bIsConsole ? Chr(0x23) & 'AutoIt3Wrapper_Change2CUI=y' : '') & @CRLF & 'OnAutoItExitRegister("__cleanup")')
...
Func _OOPE_ExtendObjects(ByRef $sCode)
	...
	For $sClsRegion In $aClsRegions
		...
		Local $sCompMethods = "Func " & $sClDecorated & '($ObjRef)' & _
				@CRLF & @TAB & 'Local Static $Members = __PointerToString(DllStructCreate($__OOPE_ObjectInstanceVariables, $ObjRef).psVarsString)' & _
				@CRLF & @TAB & 'Return DllStructCreate($Members, $ObjRef + $__OOPE_ObjectVariableOffset)' & _
				@CRLF & "EndFunc" & @CRLF & @CRLF
		Local $sMName, $sMTypeDecl, $sArgHeader, $aMArgs
		Local $sClassMethodsUser, $aClassConDes, $sConstructor, $sDestructor, $sConHasArgs

		For $sUM In $aClassMethodsUser
			$sMName = StringLeft($sUM, StringInStr($sUM, '(') - 1)
			$sMTypeDecl = $sClDecorated & $__OOPE_MethodDecorator & $sMName & " " & (StringLeft($sUM, 1) = 'f' ? 'double' : 'int64') & "("
			...
			$sCompMethods &= _
					...
					@CRLF & @TAB & 'Local $This = ' & $sClDecorated & '($___selfObjRef)' & _
	...
	For $sCDT In $aClassDefTags
		...
		For $k = 0 To UBound($aInstances) - 1
			...
			$sClsDimStr &= _
					Chr(0x23) & 'forcedef ' & $aInstances[$k][1] & @CRLF & _
					'If UBound(Execute("' & $aInstances[$k][1] & '")) Then' & @CRLF & _
					@TAB & 'For $n = 0 To UBound(Execute("' & $aInstances[$k][1] & '"))-1' & @CRLF & _
					@TAB & @TAB & $aInstances[$k][1] & '[$n] = __OOPE_InstantiateClass("' & $sOOPEArgument & ')' & _
					@CRLF & @TAB & 'Next' & @CRLF & _
					'Else' & @CRLF & _
					@TAB & $aInstances[$k][1] & ' = __OOPE_InstantiateClass("' & $sOOPEArgument & ')' & @CRLF & _
					'EndIf' & @CRLF
			$aClasses[$nClass][7] = True
			$__OOPE_GarbageCollectionBuildString &= _
					@CRLF & @TAB & Chr(0x23) & 'forcedef ' & $aInstances[$k][1] & @CRLF & _
					@TAB & 'If UBound(Execute("' & $aInstances[$k][1] & '")) Then' & @CRLF & _
					@TAB & @TAB & 'For $n = 0 To UBound(Execute("' & $aInstances[$k][1] & '"))-1' & @CRLF & _
					@TAB & @TAB & @TAB & $aInstances[$k][1] & '[$n] = 0' & @CRLF & _
					@TAB & @TAB & 'Next' & @CRLF & _
					@TAB & 'Else' & @CRLF & _
					@TAB & @TAB & $aInstances[$k][1] & ' = 0' & @CRLF & _
					@TAB & 'EndIf' & @CRLF
		Next
	...
	For $k = 0 To UBound($aClasses) - 1
		If $aClasses[$k][7] Then
			$sCode &= Chr(0x23) & 'Region Class ' & $aClasses[$k][0] & @CRLF
			$sCode &= $aClasses[$k][6]
			$sCode &= Chr(0x23) & 'EndRegion' & @CRLF & @CRLF
		EndIf
	Next
EndFunc
...
Func __OOPE_InstantiateClass($sObjVars, $sObjMethods, $xFuncConst = Default, $xFuncDest = Default, $aParamConst = Null)
	...
	Local $aClassDec = StringRegExp($sObjMethods, StringReplace($__OOPE_ClassDecoratorPattern, '%CLASSNAME%', '[A-Za-z0-9]+', 0, 1) & $__OOPE_MethodDecorator, 1)
